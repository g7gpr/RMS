...
# --------------------------------------------------------------------
def generateTimelapse(dir_path, keep_images=False, fps=None, output_file=None, hires=False, annotate_objects='none', platepar_path=None):
    ...
    Keyword arguments:
        ...
        hires: [bool] Produce a higher-resolution movie (lower CRF). False by
            default.
        annotate: [bool] Annotate with names of identified objects. False by
            default.
    ...
    from Utils.IdentifyStars import annotateImage, getObjectNames

    print("Getting star names in mode {}".format(annotate_objects))
    if annotateImage != 'none':
        calstar_radec_name = getObjectNames(dir_path, use_calstar=False,
                                                                brightest=20, pixel_separation=100, annotate=annotate_objects, platepar_path=platepar_path)
    ...
    for file_name in ff_list:
        ...
        if annotate_objects !="none":
            img = annotateImage(file_name, calstar_radec_name, img, font)
...
def main():
    ...
    parser.add_argument('--annotate', choices=['none', 'with_calstar', 'without_calstar'],
                        help='Annotate with object names')
    ...
    if not args.output:
        ...
        suffix = "timelapse.mp4"
        if args.annotate == "none":
            suffix = "timelapse.mp4"
        elif args.annotate == "with_calstar":
            suffix = "timelapse_with_calstar.mp4"
        elif args.annotate == "without_calstar":
            suffix = "timelapse_without_calstar.mp4"
    ...
    if not args.image_files:
        ...
        try:
            ...
            generateTimelapse(dir_path=args.input_dir,
                              ...
                              hires=args.hires,
                              annotate_objects=args.annotate
            )
