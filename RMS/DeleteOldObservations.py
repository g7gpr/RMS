...
def quotaReport(archived_dir, captured_dir, capt_dir_quota, config, after=False):
    ...
    rep += ("-------------------------------------------\n")
    rep += ("Space used                              \n")
    rep += ("    Archive directory space used: {:s}GB\n".format(usedSpace(archived_dir).zfill(4)))
    rep += ("   Captured directory space used: {:s}GB\n".format(usedSpace(captured_dir).zfill(4)))
    rep += ("Quotas allowed                          \n")
    rep += ("        Total quota for RMS_data: {:s}GB\n".format(config.rms_data_quota).zfill(4))
    rep += ("      Archived directories quota: {:s}GB\n".format(config.arch_dir_quota).zfill(4))
    rep += ("                  bz2 file quota: {:s}GB\n".format(config.bz2_files_quota).zfill(4))
    rep += ("          Remaining for captured: {:s}GB\n".format(capt_dir_quota).zfill(4))
    rep += ("-------------------------------------------\n")
