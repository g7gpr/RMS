...
def quotaReport(capt_dir_quota, config, after=False):
    ...
    rep += ("--------------------------------------------\n")
    rep += ("Space used                              \n")
    rep += ("    Archive directory space used : {:07.02f}GB\n".format(usedSpace(archived_dir)))
    rep += ("   Captured directory space used : {:07.02f}GB\n".format(usedSpace(captured_dir)))
    rep += ("Quotas allowed                          \n")
    rep += ("        Total quota for RMS_data : {:07.02f}GB\n".format(config.rms_data_quota))
    rep += ("      Archived directories quota : {:07.02f}GB\n".format(config.arch_dir_quota))
    rep += ("                  bz2 file quota : {:07.02f}GB\n".format(config.bz2_files_quota))
    rep += ("          Remaining for captured : {:07.02f}GB\n".format(capt_dir_quota))
    rep += ("--------------------------------------------\n")
